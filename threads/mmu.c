...
/* 사용자 가상에 해당하는 물리적 주소를 검색합니다
 * 주소 UADDR을 pml4로 지정합니다. 커널 가상 주소를 반환합니다
 * 해당 물리적 주소에 해당하거나, null 포인터인 경우
 * UADDR이 매핑되지 않았습니다. */
void *
...
/* 사용자 가상 페이지에서 페이지 맵 레벨 4 PML4의 매핑을 추가합니다
 * 커널 가상 주소 KPAGE에 의해 식별된 물리적 프레임으로의 업로드.
 * UPAGE가 아직 매핑되지 않아야 합니다. KPAGE는 아마도 획득한 페이지여야 합니다
 * palloc_get_page를 사용하여 사용자 풀에서.
 * 쓰기 가능이 참이면 새 페이지가 읽기/쓰기됩니다;
 * 그렇지 않으면 읽기 전용입니다.
 * 메모리 할당이 성공하면 true를 반환하고, 메모리 할당이 성공하면 false를 반환합니다
 * 실패한. */
...
/*  PML4의 가상 페이지 VPAGE에 대한 PTE가 true인 경우 true를 반환합니다
 * 최근, 즉 PTE가 있었던 시간 사이에 액세스했습니다
 * 설치 및 마지막으로 삭제된 시간입니다. 다음과 같은 경우 false를 반환합니다
 * PML4에는 VPAGE용 PTE가 없습니다. */
