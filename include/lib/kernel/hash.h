 ...
 * 대신, 해시에 포함될 수 있는 각 구조체는 구조체 해시_엘렘 멤버를 포함해야 합니다.  모든 해시 함수는 이러한 '구조체 해시_엘렘'에서 작동합니다.  hash_entry 매크로를 사용하면 구조체 해시 엘리먼트에서 이를 포함하는 구조체 객체로 다시 변환할 수 있습니다.  이는 링크드 리스트 구현에서 사용된 것과 동일한 기법입니다.  자세한 설명은 lib/kernel/list.h를 참조하세요. */
